...
function Step()
{
	...
	if( oInstruction != null ) {
		...
		nAction = (oInstruction.action.toUpperCase() == "R" ? 1 : (oInstruction.action.toUpperCase() == "L" ? -1 : 0));
    if( nVariant == 1 && nHeadPosition == 0 && nAction == -1 ) {
      nAction = 0;  /* Can't move left when already at left-most tape cell. */
		...
		//mudei a mensagem que aparece (estetica)
		SetStatusMessage("Passo final, sem continuação.", 1 );
		sNewState = sState;
		sNewSymbol = sHeadSymbol;
		nAction = 0;
		nLineNumber = -1;
		//adicionei controle de comandos, para ficar igual como se tivesse halted, tentando mudar apenas o estado visivel
		EnableControls( false, false, false, true, true, true, true );
	}
...
/*	debug( 5, " aTokens.length: " + aTokens.length );
	...
	if( ["L","R","*"].indexOf( aTokens[3].toUpperCase() ) < 0 ) {
		oTuple.isValid = false;
...
{
	if( hRunTimer != null ) {
		SetStatusMessage( 'Pausado; clique "Rodar" ou "Passo" para resumir.');
		EnableControls( true, true, false, true, true, true, true );
		StopTimer();
	}
}
...
{
	SetStatusMessage( 'Maquina resetada. Clique "Rodar" ou "Passo" para iniciar.');
	Reset();
	EnableControls( true, true, false, true, true, true, false );
}
...
function OnLoad()
{
	...
	if( window.location.search != "" ) {
		...
		SetStatusMessage( 'Carregue ou escreva um programa de Maquina de Turing e clique em rodar! ' );
	}
}
